...
| `blogpost.html` | Landingspagina per blogbericht via `?b=<slug>` (`?id=` als fallback, wordt herschreven naar de slug). Kop-foto, verhaal met tussenkopjes (`verhaalHtml()`: lege regel = alinea, `## ` = kopje), auteurskaart, deelknoppen, "Lees ook". Alleen gepubliceerde berichten; concept/onbekend → noindex + niet-gevonden-pagina. Per bericht dynamische SEO + BlogPosting-schema via `zetSeo()` |
...
| `pp_blogs` | Array van blogberichten (sinds v0.22), gedeeld door `blog.html`, `blogpost.html` en het beheer: `{id: "bl-<timestamp>" ("seed-blog-1/2" voor de voorbeelden), titel, datum: "2026-07-15" (publicatiedatum, bepaalt sortering), samenvatting, tekst (lege regel = alinea, "## " = tussenkopje), auteur (naam van het account dat het schreef; bepaalt wie het mag bewerken), gepubliceerd: true\|false (ontbreekt = true; helper blogGepubliceerd(b)), fotos: [data-URLs of fotos/-paden bij seeds; eerste = kop-foto], aangemaaktOp, aangemaaktDoor}`. Slug-URL via `slugify(titel)` — unieke titels gebruiken |
...
1. **Seeds staan op 3 plekken**: `seedEvents` in `index.html` én `evenement.html`, en de seed in `admin/index.html` onderaan ("Seed als leeg"). Hetzelfde geldt voor `seedBlogs`: in `blog.html`, `blogpost.html` én de blog-seed in `admin/index.html`. Wijzig je een seed, doe het overal.
