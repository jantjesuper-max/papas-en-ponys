...
| `blog.html` | Blogoverzicht: alle gepubliceerde berichten uit `pp_blogs`, nieuwste eerst (het nieuwste bericht als brede uitgelichte kaart). Zelfde menubalk als de homepage met Blog actief; Blog-schema (JSON-LD) |
| `blogpost.html` | Landingspagina per blogbericht via `?b=<slug>` (`?id=` als fallback, wordt herschreven naar de slug). Kop-foto, opgemaakt verhaal (`verhaalHtml()`/`inlineOpmaak()`: lege regel = alinea, `## `/`### ` = kopje, `**vet**`, `*cursief*`, `__onderstreept__`, `==markeren==`, `[tekst](https://...)`, `- `/`1. ` lijsten, `> ` citaat, `---` lijn, `[foto:N]` = foto N midden in het verhaal — inline gebruikte foto's verschijnen niet nóg eens onderaan), auteurskaart, deelknoppen, "Lees ook". Alleen gepubliceerde berichten; concept/onbekend → noindex + niet-gevonden-pagina. **Voorbeeldmodus** `?voorbeeld=<id>` (vanuit het beheer): toont ook concepten, met banner + noindex. Per bericht dynamische SEO + BlogPosting-schema via `zetSeo()` |
| `admin/index.html` | Beheer, achter een client-side inlog (tab Beheerders beheert de accounts, elk account heeft een rol: beheerder of blogger): evenementen CRUD + foto-upload, inschrijvingen bekijken/mailen/exporteren, WhatsApp-aanmeldingen, blogberichten (tab Blog). Elk formulierpaneel heeft een ⤢-knop die het naar de volle paginabreedte vergroot (`toggleVol()`); opslaan/annuleren zet het automatisch terug |
...
| `pp_blogs` | Array van blogberichten (sinds v0.22), gedeeld door `blog.html`, `blogpost.html` en het beheer: `{id: "bl-<timestamp>" ("seed-blog-1/2" voor de voorbeelden), titel, datum: "2026-07-15" (publicatiedatum, bepaalt sortering), samenvatting, tekst (opmaak: zie de blogpost.html-rij hierboven; het beheer heeft er sinds v0.23 een opmaakbalk voor), auteur (naam van het account dat het schreef; bepaalt wie het mag bewerken), gepubliceerd: true\|false (ontbreekt = true; helper blogGepubliceerd(b)), fotos: [data-URLs of fotos/-paden bij seeds; eerste = kop-foto, andere via [foto:N] in het verhaal te zetten], aangemaaktOp, aangemaaktDoor}`. Slug-URL via `slugify(titel)` — unieke titels gebruiken |
| `pp_beheerders` | Alleen gebruikt door het beheer (sinds v0.18): array van beheerdersaccounts `{id, naam, gebruikersnaam, rol, salt, wachtwoordHash, aangemaaktOp, aangemaaktDoor}`. `rol` (sinds v0.22): `"beheerder"` of `"blogger"` — ontbreekt het veld, dan beheerder (helper `accountRol(b)`). Een blogger ziet alleen de Blog-tab (body-class `rol-blogger`) en kan alleen eigen berichten bewerken (`magBlogBewerken()`, match op `auteur`); de laatste beheerder kan niet weg of gedegradeerd worden. Wachtwoord = SHA-256(salt+":"+wachtwoord) via Web Crypto. Seed-account bij lege sleutel: `faizi` / `PapaPonys2026!`. Sessie in **sessionStorage** onder `pp_beheer_sessie`. **Let op: client-side slot, geen echte beveiliging** — broncode en localStorage zijn op het apparaat leesbaar |
...
- **Evenement- en blog-URL's**: de nette vorm is `evenement.html?e=<slug-van-titel>` resp. `blogpost.html?b=<slug-van-titel>` (gemaakt door `slugify()`, gedupliceerd op alle pagina's die het nodig hebben); `?id=` blijft als fallback werken en wordt via `history.replaceState` herschreven naar de slug. Let op: twee items met exact dezelfde titel delen een slug — de eerste wint. Gebruik dus unieke titels.

## Valkuilen (echt lezen)

1. **Seeds staan op 3 plekken**: `seedEvents` in `index.html` én `evenement.html`, en de seed in `admin/index.html` onderaan ("Seed als leeg"). Hetzelfde geldt voor `seedBlogs`: in `blog.html`, `blogpost.html` én de blog-seed in `admin/index.html`. Ook `verhaalHtml()`/`inlineOpmaak()` (blogopmaak) staan dubbel: in `blogpost.html` en in `admin/index.html` (voor het 👁️ Voorbeeld; enige verschil daar is `blogFotoSrc()` voor de fotopaden). Wijzig je een seed of de opmaak, doe het overal.
